...
## Additional original-cooker oracle evidence

Two independent rebuilds from the same unchanged 9.3.1 source are byte-identical at 124,568 bytes, SHA256 `8238078c40f7b419b2f3cc3a14f4511f1cdb6a8bce00f3589a39fbfcde32d5b1` (**CONFIRMED_BY_BYTES** for the tested runs). Compared with the shipped DX, they retain stable main render topology/attributes but differ at float precision and unresolved secondary collision descriptor sequences; the historical DX is **NOT BYTE-IDENTICAL**, and full collision-semantic equivalence remains unclaimed.

The safe source edit maps Vector C index 646 Y `+0.15` to exactly one compiled render vertex position; marker-1339 bounds are recomputed, while normals, collision/tag101, topology, colors, UVs, and draw/index bytes are unchanged. The generated DX loads without GXM and displays the edit (**CONFIRMED_BY_RUNTIME**, user observation; **CONFIRMED_BY_BYTES**). Same-build source `$chull` points map to the regenerated tag101-B hull, but that does not explain or make safe the crashing source-only hull edit.
