...
No second `$chull` runtime mutation was attempted. R-DEMO2.3 found an ordered raw name/range correspondence, but did not map the hierarchy controls or loader-created child/sibling links. See `chull-crash.md` for the bytes and static offsets. The remaining gate is to prove whether the runtime-edited 68 records span the complete recursively collected input before preparing one coherent mutation.
