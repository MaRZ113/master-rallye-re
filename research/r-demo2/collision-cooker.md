...
The source-to-DX axis transform tested is `(x,y,z) → (x,z,-y)`, the same orientation-preserving permutation/sign transform used by the demo vehicle path. For each sample, the unique C positions referenced by the sidecar `$chull` record interval were reduced to the candidate extreme set; target Rep-B points were matched back to C indices only when the caller supplies a maximum Euclidean residual. The reproduced Trooper report uses `--mapping-tolerance 0.00001`; mapping output records that threshold, rejected points, unmatched target indices, unused source indices, and ambiguous nearest pairs. Counts agree and every target vertex maps to a distinct source index in this report. The mapping rows are explicit in each JSON report.
...
R-DEMO2.5 now proves that the selected leaf and post-weld hull stream coincide for the pinned 9.3.1 offline pair; missing descendants or welded dependencies do not explain that pair. The general collector remains recursive for other models. This finding does not identify the failure instruction or independently reconstruct the different 8.4.1 runtime candidate. No new runtime mutation was prepared or attempted.
...
`tools/scanner/r_demo_hull_invariants.py` compares a baseline GXM with an offline candidate and fails closed unless the selected flat `$chull` range has the same record topology, all and only its referenced Vector-C points undergo one translation, no selected C index is shared by records outside that range, Vector A/B and all non-C bytes remain unchanged, and every source face stays nondegenerate with triangle-normal and area drift bounded to float32 noise; indexed Vector-A alignment must also remain stable.

A scratch-only 9.3.1 Trooper copy translating all 36 referenced C positions by +0.4 source X passes all ten checks, including per-face normal, area, and Vector-A alignment preservation. Candidate SHA256: `32fcbc212f226a307a3d71c43c3cbedabcf32526f86961a3bd1af16ad3c1aac4` (no runtime launch was made). It changes 126 byte values, has maximum translation residual `3.27e-8`, zero degenerate records, maximum normal angular drift `1.81e-5°`; maximum triangle-area drift `9.250953e-8`, and maximum Vector-A alignment drift `1.641e-5 degrees`. The updated report is `.research-output/r-demo2/same-build-collision-oracle/trooper-flat-translation-invariants.json`.

The original flat checker remains a source-only check. R-DEMO2.4 established the Trooper leaf hierarchy, and R-DEMO2.5 closes its post-weld dependency question for the pinned 9.3.1 pair: no hull C index participates in any proximity pair, the record/index storage remains unchanged, and all post-weld translation checks pass. The existing 9.3.1 offline analogue is distinct from the traced 8.4.1 crash asset. See `vertex-welder.md` for Path B, complete provenance, per-input hashes and the narrow hull-library follow-up. This is not runtime-safety evidence and does not authorize another identical mutation.
