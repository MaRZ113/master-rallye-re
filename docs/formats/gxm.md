...
6. The remaining hierarchy/object bytes are preserved opaque. In the exact demo-9.3.1 Trooper car.gxm, this tail is 732 bytes and contains the length-prefixed Model and mesh names in matching sidecar order. The trailing u32 span pair after each nonterminal name corresponds to the next sidecar mesh Index/Size; the final hull name has no trailing fields. The other control bytes have not been assigned semantics, and this observation is not a general hierarchy parser. See research/r-demo2/chull-crash.md. No general GXM writer or compiler is exposed.
