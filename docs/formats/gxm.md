...
The GXM files contain source-side names and length-prefixed directives such as `$paint`, `$glass`, `$perspex`, `$chull(Trooper)`, and `$cylinder_0.766_0.39`. The `$chull(Jump)` source geometry has a float-precision numerical correspondence with retail tag101 (see `research/r-demo/collision-source-map.md`). Other directive effects and exact compiler rules remain **UNKNOWN** until tested in the runtime. `research/r-demo/gxm-directives.json` retains corpus identity and byte offsets.
