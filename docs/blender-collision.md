...
R4C deliberately leaves this Blender representation read-only: the first
collision runtime candidate is generated by the canonical template writer,
not from arbitrary Blender edits.
...
The optional R4C preview-offset UI was not implemented because it is not
needed to prove the byte-level writer and could blur the read-only boundary.
