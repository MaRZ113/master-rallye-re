...
For 1,450 of 1,483 same-build, same-stem GXI/DXT candidates, a top-down RGBA-to-bottom-up BGRA transform plus the observed DXT header with CRC32 of the transformed payload recreates the complete DXT bytes. The 33 nonmatches are September font resources. This establishes **CONFIRMED_BY_CORPUS** build correspondence for exact pairs. The first-pass user observation reports 8.4.1 DXT regeneration; exact runtime-generated bytes remain unverified pending a clean repeat. Targeted demo EXE xrefs confirm source/cache code paths, but not the result for a specific file.
...
Trooper `Black-tga.gxi` recreates its original DXT byte-identically offline. A fresh scratch-generated runtime DXT remains pending. A separate `$chull`-only GXM translation candidate changes 36 exclusive hull positions, leaving the visible source mesh positions unchanged; its runtime collision effect is pending. Targeted EXE xrefs show `.gxi`/`.dxt` branches and cache Debug strings, while file-access order awaits a trace. The Win32 Debug capture helper is ready for a live control test; see `research/r-demo/runtime/`.
...
- **CONFIRMED_BY_RUNTIME (human Trooper 8.4.1 tests):** live GXM Vector C body edit and car/complete/wheel visual loader matrix. Earlier DXT regeneration is a user observation requiring a clean byte-identity repeat.
- **UNKNOWN:** universal GXM hierarchy binary controls, exact `$cylinder` formula, DX file-access role, 9.3.1 loader behavior, clean runtime DXT byte identity, and the isolated `$chull` candidate effect.

No R-DEMO result is integrated into the retail authoring pipeline yet. `research/r-demo/findings.md` tracks progress and remaining gates.
